...
function setInfo() {
  let productsToCard = JSON.parse(localStorage.getItem(`cartItems`)) || [];

  if (productsToCard.includes(id.value)) {
    Swal.fire({
      title: 'oops',
      text: `This product has already been added to your shopping cart`,
      icon: 'info',
...
<template>
  ...
  <div class="container mt-6 m-auto">
    ...
    >
      <div
        class="max-w-lg bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 flex items-start justify-around"
      >
        <div class="h100">
          <div class="flex items-start justify-around START flex-row">
            <div class="w-fit">
              <img class="rounded-t-lg px-4" :src="mainProduct.image" alt="" />
            </div>

            <div class="p-5 pb-0 w-fit">
              <h5
