<template>
  ...
  <h1 class="text-3xl font-bold mb-3 text-center">All In One</h1>
  ...
  >
    <div
      ...
      v-for="(item, index) in mainProduct"
      :key="index"
    >
      <div class="h100">
        <div>
        ...
        <div class="p-5">
          <p class="my-3">Price : {{ item.price }} $</p>
          <!-- !-----START SVG For Star Rating  -->
          <div class="stars flex items-center w-min my-1">
            <svg
              v-for="star in Math.floor(item.rating?.rate || 0)"
              :key="star"
              xmlns="http://www.w3.org/2000/svg"
              class="h-6 w-6"
              fill="rgb(245 158 11)"
